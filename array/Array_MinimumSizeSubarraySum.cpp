/*
  ...
  Two solutions: two pointer O(n) and binary search O(n*log(n))
*/
...
/* clang-format on */
int minSubArrayLen_TwoPointers(int target, vector<int>& nums);
int minSubArrayLen_BinarySearch(int target, vector<int>& nums);
...
int main() {
    ...
    cout << "two pointers: " << minSubArrayLen_TwoPointers(target, nums) << "\n";
    cout << "binary search: " << minSubArrayLen_BinarySearch(target, nums) << "\n";
}   
...
int minSubArrayLen_BinarySearch(int target, vector<int>& nums) {
    ...
    while (l <= r) {
        ...
        // Check if with current length, can we found any subarray sum >= target
        for (int i = 0; i < m; i++) {
            sum += nums[i];
        }
        if (sum >= target) {
            ...
            for (int i = 1; i + m - 1 <= nums.size() - 1; i++) {
                sum -= nums[i - 1];
                sum += nums[i + m - 1];
                if (sum >= target) {isValid = true; break;}
            }
        }
