/*
  ...
  This is in-place O(1) space solution
*/
...
void rotate(vector<int>& nums, int k) {
    // Imagine divide the array before swapping XY
    // After swapping YX
    // => Problem swap from XY -> YX without extra array
    // X: index 0 -> (n - 1 - k)        (last element index + k = n - 1)
    // Y have length k

    // Move n time the same as move 0 => k = k%n;
    k = k%nums.size();                          // Ori array:   1,2,3,4, 5,6,7
    reverse(nums.begin(), nums.end());          // Swap array:  7,6,5, 4,3,2,1
    reverse(nums.begin(), nums.begin() + k);    // Swap Y:      5,6,7, 4,3,2,1
    reverse(nums.begin() + k, nums.end());      // Swap X:      5,6,7, 1,2,3,4
}
