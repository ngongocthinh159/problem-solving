...
// One thing to notice in this problem:
// If we choose to start from position A
// Then if position A -> B -> C --- D (but cannot reach D)
// Then B and C also can not reach D. Why?

// Think of when we start from C to reach D:
// Previously, if we can reach C from A => Mean that the total gas gain in A and B must be at least >= 0
// Hence going from A instead of C will improve our chance to reach D
// But like we have seen, we cannot reach D even if we start from A
// ====> So B and C also can not reach

// Then we can ignore to assess B and C
// We can start to assess if D is a good starting point
int canCompleteCircuit(vector<int>& gas, vector<int>& cost) {
    int i = 0;
    int res = -1;
    while (i < cost.size()) {
        ...
        if (sum < 0) i++;
            ...
            // Move until the sum < 0
            while (sum + gas[(i + j)%cost.size()] - cost[(i+j)%cost.size()] >= 0) {
                sum += gas[(i + j)%cost.size()] - cost[(i+j)%cost.size()];
                if (j == cost.size()) {
                    res = i;
                    isFound = true;
                    break;
            ...
            // If not found => Move i to the position can reach to as new potential start point
            i += j;
        }
    }
    return res;
