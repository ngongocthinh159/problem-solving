/*
  Solution for: https://leetcode.com/problems/minimum-number-of-arrows-to-burst-balloons
*/
...
// For each balloon, we at least must shoot 1 time in range [balloon[0], balloon[1]]
// So we will try to greedy: for each ballon, choose the position the get us the most balloons
// To choose the greedy position, we must sort the balloon in the order of start position
int findMinArrowShots(vector<vector<int>>& points) {
    ...
    for (int i = 1; i < points.size(); i++) {
        ...
        // Overlap case 
        // => Narrow down the range that allow to shoot multiple balloons at the same time
        if (!((curEnd < start) || (end < curStart))) {
            start = max(start, curStart);
            end = min(end, curEnd);

        // The range of current balloon is not in the range of the allowed overlapped range
        // => Jump: New range must be ebstablished (~ new arrow)
        } else {
            cnt++;
            start = curStart;
            end = curEnd;
    ...
    // For the last point: 
    // either the last point in the overlap case => cnt not count for this overlap interval
    // or the last point establish a new range => cnt only count for the previous range, not the new newly established range
    cnt++;
    return cnt;
}
